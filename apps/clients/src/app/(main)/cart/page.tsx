'use client'
import PaymentForm from '@/components/Home/cart/PaymentForm';
import ShippingForm from '@/components/Home/cart/ShippingForm';
        ...
        <div className='flex flex-col gap-8 items-center justify-center mt-12'>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-16">
                {steps.map(step => (
                    <div className={`flex items-center gap-2 border-b-2 pb-4 ${step.id === activeStep ? 'border-gray-800' : "border-gray-200"}`}
                        ...
                        <p className={`text-sm font-medium ${step.id === activeStep ?
                            'text-gray-800' : "text-gray-400"}`}>{step.title}</p>
                    </div>
                ))}
            </div>
            {/* step và details*/}
            <div className='w-full flex flex-col lg:flex-row gap-16'>
                ...
                <div className="w-full lg:w-7/12 shadow-lg border-1 border-gray-100 p-8 rounded-lg flex flex-col gap-8">
                    {
                        activeStep === 1 ? (
                                    ...
                                    <div className="flex gap-8">
                                        ...
                                        <div className="flex flex-col justify-between">
                                            <div className="flex flex-col justify-between">
                                                ...
                                                <p className='text-sm text-gray-500'>Sizes:{" "}{item.sizes}</p>
                            ...
                            activeStep === 2 ? (
                                ...
                                <p className="text-sm font-medium text-gray-800">Please fill in the shipping form to continue.</p>
                            )
                    }
                </div>
                {/* details */}
                <div className="w-full lg:w-5/12 shadow-lg border-1 border-gray-100 p-8 rounded-lg flex flex-col gap-8 h-max">
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex justify-between text-sm">
                            ...
                            <p className=' font-medium'>${cart.reduce(
                                (acc, item) => acc + item.price * item.quantity, 0).toFixed(2)}</p>
                        </div>
                    ...
                    {activeStep === 1 && (
                        < Button onClick={() => router.push("/cart?step=2", { scroll: false })} className='cursor-pointer'>
                            Continue
